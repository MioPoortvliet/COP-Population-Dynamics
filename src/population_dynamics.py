...
class AnimalEvolution:
    ...
    def adjacent_interactions(self, animal) -> None:
        ...
        for neighbour_idx in neighbours_idx:
            neighbour_pos = tuple(
                neighbour_idx)  # there is a difference between a tuple and an array when slicing/indexing
            ...
            for direction in range(4):
                ...
                if self.animal_map[baby_position] == 0:
                    ...
                    newanimal = animal_type(
                        ...
                        # mean_max_hunger just diverges but that is unrealistic
    ...
    def run_cycles(self, maxcycles=10, printskip=15) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for cycle in range(maxcycles):
            ...
            if np.any(self.population[cycle - 1,
                      1::] <= 0) and cycle != 0:  # check if there are less than two animals of any kind.
                ...
                # Otherwise force at least two animals of every kind
                    ...
                    for i, animal_key in enumerate(self.animal_objects.keys()):
                        ...
                        if self.population[cycle - 1, i + 1] <= 1 and self.settings[animal_key] > self.population[
                            cycle - 1, i + 1]:
                            ...
                            self.position_entities_on_map(entities=[animal], positions=[tuple(position)],
                                                          map=self.animal_map)
