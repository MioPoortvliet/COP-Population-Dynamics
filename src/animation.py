...
class MapAnimation:
    def __init__(self, simulation, frameskip=1):
        """
        Initializes the Animation Class. Stores and calculates all relevant physical quantities and sets up the
        figures for the animation.
        ...
        self.lines_animal_population = [self.ax_animal_population.plot(self.stats[::, i]) for i in
                                        range(1, self.stats.shape[1])]
        ...
        self.lines_rabbit_1 = [self.ax_rabbit_1.plot(self.genes[::, 1, i, 0], label=label) for i, label in
                               enumerate(self.genes_labels[0:3])]

        self.ax_fox_1 = self.figure.add_subplot(self.gs[1:3, n:2 * n])
        self.lines_fox_1 = [self.ax_fox_1.plot(self.genes[::, 0, i, 0], label=label) for i, label in
                            enumerate(self.genes_labels[0:3])]

        self.ax_rabbit_2 = self.figure.add_subplot(self.gs[4:, 0:n])
        self.lines_rabbit_2 = [self.ax_rabbit_2.plot(self.genes[::, 1, i + 3, 0], label=label) for i, label in
                               enumerate(self.genes_labels[3:])]

        self.ax_fox_2 = self.figure.add_subplot(self.gs[4:, n:2 * n])
        self.lines_fox_2 = [self.ax_fox_2.plot(self.genes[::, 0, i + 3, 0], label=label) for i, label in
                            enumerate(self.genes_labels[3:])]

    def update(self, j):
        """
        Runs the symulation for [frameskip] cycles and then updates the figure. To be called by the FuncAnimation
        function.
        ...
        [ax.clear() for ax in
         [self.ax_animal_population, self.ax_food_population, self.ax_rabbit_1, self.ax_fox_1, self.ax_rabbit_2,
          self.ax_fox_2]]
        ...
        self.lines_rabbit_1 = [self.ax_rabbit_1.plot(self.genes[::, 1, i + self.labels[0], 0], label=label) for i, label
                               in enumerate(self.genes_labels[0:3])]
        self.lines_fox_1 = [self.ax_fox_1.plot(self.genes[::, 0, i, 0], label=label) for i, label in
                            enumerate(self.genes_labels[0:3])]
        self.lines_rabbit_2 = [self.ax_rabbit_2.plot(self.genes[::, 1, i + 3, 0], label=label) for i, label in
                               enumerate(self.genes_labels[3:])]
        ...
        [ax.set_ylim(0) for ax in
         [self.ax_animal_population, self.ax_food_population, self.ax_rabbit_1, self.ax_fox_1, self.ax_rabbit_2,
          self.ax_fox_2]]
        [ax.set_xlim(0) for ax in
         [self.ax_animal_population, self.ax_food_population, self.ax_rabbit_1, self.ax_fox_1, self.ax_rabbit_2,
          self.ax_fox_2]]
